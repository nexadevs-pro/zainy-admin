...
const formSchema = z.object({
  ...
  description1: z.string().min(2).max(500).trim(),
  description2: z.string().min(2).max(500).trim(),
  description3: z.string().min(2).max(500).trim(),
  image: z.string(),
});
...
const BlogForm: React.FC<BlogFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (res.ok) {
        setLoading(false);
        toast.success(`Blogs ${initialData ? "updated" : "created"}`);
